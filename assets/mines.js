...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a18005fc";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a18005fc">'+'<span class="tile-face-a18005fc tile-front-a18005fc"></span>'+'<span class="tile-face-a18005fc tile-back-a18005fc"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a18005fc")){return;}
var back=tile.querySelector(".tile-back-a18005fc");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a18005fc");if(isMine){tile.classList.add("boom-a18005fc");}
if(dim){tile.classList.add("dimmed-a18005fc");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a18005fc")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a18005fc");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a18005fc")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a18005fc");void panelEl.offsetWidth;panelEl.classList.add("jolt-a18005fc");}
endRound();}
